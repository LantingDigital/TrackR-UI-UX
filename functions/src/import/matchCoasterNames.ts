...
interface MatchInput {
  /** Array of unique coaster names to match */
  names: Array<{
    rawName: string;
    rawParkName: string | null;
  }>;
  /** The coaster database to match against (sent from client) */
 ...
 */
function combinedSimilarity(imported: string, database: string): number {
 ...
 */
function parkBoost(
  importedPark: string | null,
  databasePark: string,
): number {
  if (!importedPark) return 0;
  const sim = stringSimilarity(importedPark, databasePark);
  if (sim >= 0.7) return 0.1; // Strong park match
  if (sim >= 0.5) return 0.05; // Partial park match
  return 0;
}
...
export const matchCoasterNames = onCall(
  {
    region: 'us-central1',
    memory: '1GiB', // Coaster DB can be large
    timeoutSeconds: 120,
  },
  async (request): Promise<MatchOutput> => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Must be signed in.');
    }

    const data = request.data as MatchInput;

    if (!data.names || !Array.isArray(data.names)) {
      throw new HttpsError('invalid-argument', 'names array is required.');
    }

    if (
      !data.coasterDatabase ||
      !Array.isArray(data.coasterDatabase) ||
      data.coasterDatabase.length === 0
    ) {
      throw new HttpsError(
        'invalid-argument',
        'coasterDatabase array is required.',
      );
    }
    ...
    const results: CoasterMatchResult[] = [];
    ...
    for (const input of data.names) {
      ...
      // Score every coaster
      const scored: Array<CoasterMatch & { score: number }> = [];

      for (const entry of dbEntries) {
        // Quick skip: if normalized lengths differ by more than 3x, unlikely match
        if (
          entry.normalized.length > normalizedInput.length * 3 ||
    ...
    console.log(
      `[matchCoasterNames] User ${request.auth.uid}: matched ${data.names.length} names against ${data.coasterDatabase.length} coasters`,
    );
