...
interface ProcessImportInput {
  ...
  fileBase64: string;
  /** File extension hint (e.g., 'csv', 'xlsx', 'json') */
...
interface ImportedRide {
  ...
  rawDate: string | null;
  parsedDate: string | null; // ISO 8601
...
function parseCSV(content: string): { headers: string[]; rows: RawRow[] } {
...
function parseTSV(content: string): { headers: string[]; rows: RawRow[] } {
  const result = Papa.parse<RawRow>(content, {
    header: true,
    skipEmptyLines: true,
    delimiter: '\t',
...
function parseExcel(buffer: Buffer): { headers: string[]; rows: RawRow[] } {
...
function parseJSON(content: string): { headers: string[]; rows: RawRow[] } {
  ...
  throw new Error(
    'JSON must be an array of objects, or an object with a "rides", "data", or "logs" array.',
  );
}
...
export const processImportFile = onCall(
  {
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 60,
  },
  async (request): Promise<ImportParseResult> => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Must be signed in.');
    }

    const data = request.data as ProcessImportInput;
    ...
    try {
        ...
        default:
          ...
          try {
            ...
            if (headers.length > 0 && rows.length > 0) {
              warnings.push(
                'File format was unrecognized. Attempted to parse as CSV.',
              );
            } else {
              throw new HttpsError(
                'invalid-argument',
                "We couldn't read this file. Try exporting as CSV or Excel.",
              );
            }
          } catch {
            throw new HttpsError(
      ...
      throw new HttpsError(
        'invalid-argument',
        `Failed to parse file: ${(e as Error).message}`,
      );
    }

    if (rows.length === 0) {
      throw new HttpsError('invalid-argument', 'File contains no data rows.');
    }
    ...
    if (!assignedFields.has('coasterName')) {
      throw new HttpsError(
        'invalid-argument',
        'Could not identify a coaster name column. Please ensure your file has a column for coaster/ride names.',
      );
    }
    ...
    if (rides.length === 0) {
      throw new HttpsError(
        'invalid-argument',
        'No valid ride entries found in the file.',
      );
    }

    // Warn about unparseable dates
    const unparsedDates = rides.filter(
      (r) => r.rawDate && !r.parsedDate,
    ).length;
    if (unparsedDates > 0) {
      warnings.push(
        `${unparsedDates} date(s) could not be parsed and will need manual entry.`,
      );
    }

    console.log(
      `[processImportFile] User ${request.auth.uid}: ${rides.length} rides parsed from ${format} file "${data.fileName}"`,
    );
