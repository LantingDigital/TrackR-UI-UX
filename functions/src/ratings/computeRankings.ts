...
const TIME_WINDOWS = ['all-time', 'this-year', 'this-month'] as const;

export const computeRankings = onSchedule(
  {
  ...
  async () => {
    ...
    for (const doc of allRatings.docs) {
      ...
      if (!data.coasterId || !data.criteriaRatings) continue;

      const coasterId = data.coasterId as string;
      const coasterName = (data.coasterName as string) || '';
      const parkName = (data.parkName as string) || '';
      const criteriaRatings = data.criteriaRatings as Record<string, number>;
      const weightedScore = (data.weightedScore as number) || 0;
      ...
      const applicableWindows: Array<typeof TIME_WINDOWS[number]> = [
        'all-time',
      ];
      if (isThisYear) applicableWindows.push('this-year');
      if (isThisMonth) applicableWindows.push('this-month');

      for (const window of applicableWindows) {
        // Overall category uses weightedScore
        addToAggregation(
          aggregations.get(`overall_${window}`)!,
          coasterId,
          coasterName,
          parkName,
          weightedScore,
        ...
        for (const category of CATEGORIES) {
          ...
          if (score != null && score > 0) {
            addToAggregation(
              aggregations.get(`${category}_${window}`)!,
              coasterId,
              coasterName,
              parkName,
              score * 10, // Normalize 0-10 to 0-100 for consistent storage
            );
          }
        }
      }
    ...
    const prevRankings = new Map<string, Map<string, number>>();
    for (const category of CATEGORIES) {
      const prevDoc = await db
        .doc(`rankings/${category}_all-time`)
        .get();
      if (prevDoc.exists) {
        const entries = (prevDoc.data()?.entries as Array<{
          coasterId: string;
          rank: number;
        }>) ?? [];
    ...
    console.log(
      `[computeRankings] Computed ${batchOps} ranking docs from ${allRatings.size} ratings`,
    );
  },
);

function addToAggregation(
  map: Map<string, RatingAggregation>,
  coasterId: string,
  coasterName: string,
  parkName: string,
  score: number,
