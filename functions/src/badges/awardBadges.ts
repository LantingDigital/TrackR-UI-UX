...
type BadgeEvent = 'ride-logged' | 'rating-created' | 'game-completed';
...
interface BadgeDefinition {
  ...
  icon: string;
  tier: BadgeTier;
  event: BadgeEvent; // which event triggers this check
  check: (stats: UserStats) => { earned: boolean; progress: number };
}
...
const BADGE_CATALOG: BadgeDefinition[] = [
  ...
  // Game milestones
  {
    ...
    check: (s) => {
      const gamesPlayed = Object.keys(s.gameStats).filter(
        (g) => s.gameStats[g].gamesPlayed > 0,
      ).length;
  ...
  },
  {
    ...
    event: 'game-completed',
    check: (s) => {
      const maxStreak = Object.values(s.gameStats).reduce(
        (max, g) => Math.max(max, g.bestStreak),
        0,
      );
...
async function collectUserStats(
  ...
  const logsSnap = await db
    ...
    .select('parkName')
    .get();
  const distinctParks = new Set(
    logsSnap.docs.map((d) => d.data().parkName as string),
  ).size;
  ...
  return {
    totalCredits: (userData.totalCredits as number) ?? 0,
    totalRides: (userData.totalRides as number) ?? 0,
    totalRatings,
    distinctParks,
    gameStats,
    eventContext: eventContext as UserStats['eventContext'],
  };
}
...
export async function checkAndAwardBadges(
  ...
  badgesSnap.forEach((doc) => {
    const data = doc.data();
    if ((data.progress as number) >= 100) {
    ...
    if (result.earned) {
      ...
      try {
        await sendNotificationToUser(
          ...
          { screen: 'profile', badgeId: badge.id },
        );
      } catch (e) {
