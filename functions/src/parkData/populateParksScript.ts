...
if (getApps().length === 0) {
  ...
  if (serviceAccountPath) {
    ...
    initializeApp({ projectId: 'trackr-coaster-app' });
  }
}
...
function generateMockLocation(center: { lat: number; lng: number }) {
      ...
      try {
        ...
        for (const child of children) {
          ...
          if (child.entityType === 'ATTRACTION') {
            batch.set(
              db.doc(`parks/${parkDocId}/attractions/${child.id}`),
              {
                name: child.name,
                entityId: child.id,
                coordinates: coords,
            ...
            batch.set(
              db.doc(`parks/${parkDocId}/restaurants/${child.id}`),
              {
                name: child.name,
                entityId: child.id,
                coordinates: coords,
            ...
            batch.set(
              db.doc(`parks/${parkDocId}/shows/${child.id}`),
              {
                name: child.name,
                entityId: child.id,
                coordinates: coords,
        ...
        console.log(
          `  ${parkEntity.name}: ${aCount}A ${rCount}R ${sCount}S (${children.length} total)`,
        );
