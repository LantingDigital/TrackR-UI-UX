/**
 * Populate Firestore using Firebase CLI's refresh token for auth.
 *
 * Run: cd functions && npx ts-node src/parkData/runPopulateWithCliAuth.ts
 */
...
if (!refreshToken) {
  console.error('No refresh token found in Firebase CLI config. Run: firebase login --reauth');
  process.exit(1);
}

// Initialize Firebase Admin with the refresh token
if (getApps().length === 0) {
  ...
  // Create a credential object from the auth client
  const credential = {
...
async function main() {
  ...
  // Quick test: try writing a test doc
  console.log('Testing Firestore connection...');
  try {
    await db.doc('_test/connection').set({ ok: true, ts: FieldValue.serverTimestamp() });
    await db.doc('_test/connection').delete();
