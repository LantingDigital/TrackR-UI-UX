...
export interface ParkIdMapping {
  /** TrackR internal park slug */
  trackrSlug: string;
  /** Queue-Times.com numeric park ID */
  queueTimesId: number;
  /** ThemeParks.wiki entity GUID */
  themeparksWikiId: string;
  /** ThemeParks.wiki destination GUID (parent) */
  destinationId: string;
  /** Park name (from ThemeParks.wiki) */
  name: string;
}

export const PARK_ID_MAPPING: ParkIdMapping[] = [
  // === Cedar Fair ===
  {
...
// Lookup helpers
const bySlug = new Map<string, ParkIdMapping>();
const byThemeParksWikiId = new Map<string, ParkIdMapping>();
const byQueueTimesId = new Map<number, ParkIdMapping>();

for (const m of PARK_ID_MAPPING) {
  bySlug.set(m.trackrSlug, m);
  byThemeParksWikiId.set(m.themeparksWikiId, m);
  byQueueTimesId.set(m.queueTimesId, m);
}

export function getMappingBySlug(slug: string): ParkIdMapping | undefined {
  return bySlug.get(slug);
}

export function getMappingByThemeParksWikiId(id: string): ParkIdMapping | undefined {
  return byThemeParksWikiId.get(id);
}

export function getMappingByQueueTimesId(id: number): ParkIdMapping | undefined {
