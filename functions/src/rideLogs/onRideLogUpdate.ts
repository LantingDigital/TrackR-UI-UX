...
export const onRideLogUpdate = onDocumentUpdated(
  {
  ...
  async (event) => {
    ...
    const oldTimestamp = before.timestamp as string;
    const newTimestamp = after.timestamp as string;
    ...
    const coasterId = after.coasterId as string;
    ...
    console.log(
      `[onRideLogUpdate] User ${userId}: timestamp changed from ${oldDate} to ${newDate}`,
    );
  },
);
 ...
 */
async function recomputeDayRideCounts(
  ...
  logsOnDay.forEach((doc, index) => {
    const currentCount = doc.data().rideCount as number;
