...
export const onRideLogCreate = onDocumentCreated(
  {
  ...
  async (event) => {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      const newTotalRideCount = (currentMeta.totalRideCount as number) + 1;
      const newCreditCount =
        (currentMeta.creditCount as number) + (isNewCredit ? 1 : 0);
    ...
    console.log(
      `[onRideLogCreate] User ${userId}: totalRides+1, coasterId=${coasterId}`,
    );

    // Check badge criteria (best-effort, don't fail the trigger)
    try {
      await checkAndAwardBadges(userId, 'ride-logged');
    } catch (e) {
