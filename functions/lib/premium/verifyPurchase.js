...
async function verifyWithApple(receipt, useSandbox) {
    ...
    if (!response.ok) {
        throw new https_1.HttpsError('internal', 'Apple verification service unavailable.');
    }
...
exports.verifyPurchase = (0, https_1.onCall)({
    ...
    if (!productInfo) {
        throw new https_1.HttpsError('invalid-argument', `Unknown productId: ${productId}`);
    }
    ...
    if (appleResponse.status !== 0) {
        console.error(`[verifyPurchase] Apple returned status ${appleResponse.status} for user ${uid}`);
        throw new https_1.HttpsError('failed-precondition', `Receipt verification failed (status: ${appleResponse.status}).`);
    }
    // Find the matching transaction
    const transactions = appleResponse.latest_receipt_info ?? [];
    const matchingTx = transactions.find((tx) => tx.product_id === productId);
    if (!matchingTx) {
        throw new https_1.HttpsError('failed-precondition', 'No matching transaction found in receipt.');
    }
    const expiresAtMs = parseInt(matchingTx.expires_date_ms, 10);
    const expiresAt = new Date(expiresAtMs);
    const isExpired = expiresAt.getTime() < Date.now();
    if (isExpired) {
        throw new https_1.HttpsError('failed-precondition', 'Subscription has already expired.');
    }
    ...
    console.log(`[verifyPurchase] User ${uid} verified as Pro (${tier}, $${productInfo.price}/${productInfo.isAnnual ? 'yr' : 'mo'})`);
    // Send welcome notification on first Pro subscription
    if (isFirstPro) {
        try {
            await (0, sendNotification_1.sendNotificationToUser)(uid, 'Welcome to TrackR Pro!', 'Thanks for supporting TrackR. Your premium features are now active.', { screen: 'settings' });
        }
        catch (e) {
