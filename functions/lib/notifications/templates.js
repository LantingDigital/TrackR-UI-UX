 ...
 */
function getNotificationContent(type, vars = {}) {
    const template = TEMPLATES[type];
    const title = typeof template.title === 'function' ? template.title(vars) : template.title;
    const body = typeof template.body === 'function' ? template.body(vars) : template.body;
 ...
 */
async function sendNotificationFromTemplate(userId, type, vars = {}) {
    const { title, body, data } = getNotificationContent(type, vars);
    await (0, sendNotification_1.sendNotificationToUser)(userId, title, body, data);
}
