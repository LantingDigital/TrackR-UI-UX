...
exports.refreshParkHoursManual = exports.refreshParkHoursScheduled = void 0;
...
function buildDisplayString(open, close, timezone, isClosed) {
    if (isClosed)
        return 'Closed today';
    if (!open || !close)
...
// Core Logic
// ============================================
async function refreshHoursForAllParks() {
    ...
    for (const parkDoc of parksSnap.docs) {
        ...
        try {
            ...
            const schedule = await (0, themeparksWiki_1.getEntitySchedule)(parkId);
            ...
            for (const [date, dayData] of byDate) {
                ...
                const hoursDoc = {
                    ...
                    timezone,
                    displayString: buildDisplayString(dayData.operating?.open || null, dayData.operating?.close || null, timezone, isClosed),
                    isClosed,
...
// ============================================
exports.refreshParkHoursScheduled = (0, scheduler_1.onSchedule)({
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 540,
    schedule: '0 5 * * *',
    ...
    console.log(`[refreshParkHours] Done. Processed: ${result.processed}, Errors: ${result.errors.length}`);
...
// ============================================
exports.refreshParkHoursManual = (0, https_1.onCall)({
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 540,
}, async (request) => {
...
//# sourceMappingURL=refreshParkHours.js.map