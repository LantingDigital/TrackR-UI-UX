/**
 * SpeedSorterSettingsContent — Three-screen settings card
 *
 * Screen 0: Main Settings (Statistics →, Hard Mode, Show Timer, About →)
 * Screen 1: Statistics (Games Played, Best Score, Best Time, Perfect Rounds)
 * Screen 2: About
 */
...
import type { SpeedSorterStats } from '../types/speedSorter';
import type { SpeedSorterSettings } from '../stores/speedSorterStore';

const { width: SCREEN_WIDTH } = Dimensions.get('window');
const CARD_WIDTH = SCREEN_WIDTH - 64;

interface SpeedSorterSettingsContentProps {
  stats: SpeedSorterStats;
  settings: SpeedSorterSettings;
  onSetHardMode: (v: boolean) => void;
  onSetShowTimer: (v: boolean) => void;
  close: () => void;
}
...
export const SpeedSorterSettingsContent: React.FC<SpeedSorterSettingsContentProps> = ({
  stats, settings, onSetHardMode, onSetShowTimer, close,
}) => {
          ...
          <GameSettingsRow
            ...
            trailing={
              <Switch
                ...
                onValueChange={(v) => { onSetShowTimer(v); haptics.tap(); }}
                trackColor={{ false: colors.border.subtle, true: colors.accent.primary }}
                thumbColor={colors.text.inverse}
                ios_backgroundColor={colors.border.subtle}
              />
      ...
      <Reanimated.View style={[StyleSheet.absoluteFill, styles.subScreenOverlay, statsStyle]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} bounces={false} contentContainerStyle={styles.scrollContent}>
          <View style={styles.statsRow}>
            ...
            <GameStatCard label="Best Score" value={stats.bestScore > 0 ? `${stats.bestScore}%` : '—'} />
            <GameStatCard label="Best Time" value={formatTime(stats.bestTime)} />
            <GameStatCard label="Perfect" value={stats.perfectRounds} />
          </View>
          <View style={styles.bottomPad} />
        </ScrollView>
      </Reanimated.View>
