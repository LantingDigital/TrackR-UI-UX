/**
 * BlindRankingSettingsContent — Three-screen settings card
 *
 * Screen 0: Main Settings (Statistics →, Show Community Comparison, About →)
 * Screen 1: Statistics (Games Played, per-category play counts)
 * Screen 2: About
 */
...
import { BLIND_RANKING_CATEGORIES } from '../stores/blindRankingStore';

const { width: SCREEN_WIDTH } = Dimensions.get('window');
const CARD_WIDTH = SCREEN_WIDTH - 64;

interface BlindRankingSettingsContentProps {
  stats: BlindRankingStats;
  settings: BlindRankingSettings;
  onSetShowCommunityComparison: (v: boolean) => void;
  close: () => void;
}

export const BlindRankingSettingsContent: React.FC<BlindRankingSettingsContentProps> = ({
  stats, settings, onSetShowCommunityComparison, close,
}) => {
      ...
      <Reanimated.View style={[StyleSheet.absoluteFill, screen0Style]}>
        <ScrollView showsVerticalScrollIndicator={false} bounces={false} contentContainerStyle={styles.scrollContent}>
          ...
          <GameSettingsRow
            ...
            sublabel="Show in results"
            trailing={
              <Switch
                value={settings.showCommunityComparison}
                onValueChange={(v) => { onSetShowCommunityComparison(v); haptics.tap(); }}
      ...
      <Reanimated.View style={[StyleSheet.absoluteFill, styles.subScreenOverlay, statsStyle]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} bounces={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.categoryStats}>
            {BLIND_RANKING_CATEGORIES.map((cat) => (
              <View key={cat.id} style={styles.catStatRow}>
                <Text style={styles.catStatName}>{cat.title}</Text>
                <Text style={styles.catStatValue}>{stats.categoryPlays[cat.id] ?? 0}</Text>
              </View>
            ))}
          </View>
