...
export function FoodListView({ visible, onClose, pois }: FoodListViewProps) {
  ...
  // Filter to food only
  const foodPois = useMemo(
    () => pois.filter(p => p.type === 'food'),
    [pois],
  );

  // Search filters by name AND menuItems
  const sections = useMemo<FoodSection[]>(() => {
    const query = searchQuery.toLowerCase().trim();
    const filtered = query.length > 0
      ? foodPois.filter(f => {
          if (f.name.toLowerCase().includes(query)) return true;
          if (f.menuItems?.some(item => item.toLowerCase().includes(query))) return true;
          if (f.menuDescription?.toLowerCase().includes(query)) return true;
          return false;
        })
      : foodPois;
    ...
    for (const food of filtered) {
      const existing = groups.get(food.area) ?? [];
      existing.push(food);
      groups.set(food.area, existing);
    }
  ...
  }, [foodPois, searchQuery]);

  // Flat list data: interleave section headers with food rows
  const listData = useMemo(() => {
    const data: Array<{ type: 'header'; area: string; label: string; count: number } | { type: 'food'; poi: ParkPOI }> = [];
    for (const section of sections) {
      data.push({ type: 'header', area: section.area, label: section.areaLabel, count: section.spots.length });
      for (const spot of section.spots) {
        data.push({ type: 'food', poi: spot });
      }
    }
    return data;
  }, [sections]);

  const totalFoodCount = foodPois.length;
  ...
  const handleFoodPress = useCallback((poiId: string) => {
    openPOI(poiId);
  }, [openPOI]);
          ...
          {contentReady && listData.length > 0 ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyTitle}>
                {searchQuery.length > 0 ? 'No food found' : 'No food data'}
              </Text>
              <Text style={styles.emptySubtitle}>
                {searchQuery.length > 0
                  ? `No restaurants match "${searchQuery}"`
                  : 'Food data is not available for this park yet'}
              </Text>
            </View>
          ) : (
