 ...
 *
 * A stripped copy of CoasterSheet (src/features/parks/components/CoasterSheet.tsx)
 ...
 *  - haptics calls removed (harmless but unnecessary in demo)
 *
 * Added:
 *  - autoScrollDelay prop — after opening, auto-scrolls the sheet content
 *  - scrollRef exposed for programmatic control
 */
...
// These must sum correctly so imageHeight = sheetHeight - HERO_PAGE_CHROME
// fills the remaining space and stats sit flush at the bottom edge.
const HANDLE_AREA = 33;
const SCROLL_PAD_TOP = 6;
const HEADER_EST = 120;       // name + park + meta + status badge (very generous)
const IMAGE_MARGIN = 6;       // minimal
const STATS_MARGIN = 6;       // minimal
const STATS_ROW_HEIGHT = 185; // very generous — stat pills + labels + comfortable breathing room
...
export interface OnboardingCoasterSheetRef {
  scrollTo: (y: number, animated?: boolean) => void;
  scrollToAdditionalInfo: () => void;
}

interface OnboardingCoasterSheetProps {
  coaster: EnrichedCoaster | null;
  visible: boolean;
  onClose: () => void;
  /** Delay in ms before auto-scrolling the sheet content after open. 0 = no auto-scroll. */
  autoScrollDelay?: number;
  /** How far to auto-scroll (default 200) */
  autoScrollY?: number;
}

export const OnboardingCoasterSheet = forwardRef<OnboardingCoasterSheetRef, OnboardingCoasterSheetProps>(
  function OnboardingCoasterSheet({ coaster, visible, onClose, autoScrollDelay = 0, autoScrollY = 200 }, ref) {
    ...
    // Expose scrollTo + heroPageEnd via ref
    useImperativeHandle(ref, () => ({
      scrollTo: (y: number, animated = true) => {
        scrollRef.current?.scrollTo({ y, animated });
      },
      scrollToAdditionalInfo: () => {
        // Single fast scroll to exact snap point — snapToOffsets handles the landing
        scrollRef.current?.scrollTo({ y: heroPageEnd, animated: true });
      },
    }));
      ...
      if (visible && coaster) {
        ...
        if (autoScrollDelay > 0) {
          const timer = setTimeout(() => {
            scrollRef.current?.scrollTo({ y: autoScrollY, animated: true });
          }, autoScrollDelay);
        ...
        >
          ...
          <ScrollView
            ...
            contentContainerStyle={[
              ...
              { minHeight: heroPageEnd + (sheetHeight - HANDLE_AREA - 100) },
            ]}
...
// ============================================
// Styles (identical to CoasterSheet)
// ============================================
