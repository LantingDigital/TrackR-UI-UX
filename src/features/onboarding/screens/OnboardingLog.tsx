/**
 * OnboardingLog — Screen 3 of onboarding.
 * Same embedded approach as OnboardingSearch but demos the Log morph flow.
 * Auto-demos: Log button → search coaster → tap → "Ride Logged!" celebration → close → loop
 */
...
export const OnboardingLog: React.FC<OnboardingScreenProps> = ({ isActive }) => {
  const insets = useSafeAreaInsets();

  return (
    <View style={styles.container}>
      <View style={[styles.textRegion, { paddingTop: insets.top + spacing.xxxl + spacing.lg }]}>
        <Text style={styles.title}>Log Every Ride</Text>
        ...
        <View style={styles.phoneFrame}>
          ...
          <View style={styles.phoneInner}>
            <View
              ...
              pointerEvents="none"
            >
              <OnboardingSearchEmbed isActive={isActive} demoMode="log" />
            </View>
          </View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  phoneFrame: { width: PHONE_WIDTH, height: PHONE_HEIGHT, borderRadius: PHONE_RADIUS, borderWidth: PHONE_BORDER, borderColor: '#2C2C2E', backgroundColor: colors.background.page, overflow: 'hidden', ...shadows.section },
