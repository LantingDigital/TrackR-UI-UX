...
import { SimulatedLocationMarker } from './SimulatedLocationMarker';
import { DevLocationBanner } from './DevLocationBanner';
import { useDirections } from './useDirections';
import { useGeoFence } from './useGeoFence';
import { useSimulatedLocation, SIMULATION_ENABLED } from './useSimulatedLocation';
...
import { useWalkwayEditor, WalkwayEditorLayers, WalkwayEditorToolbar } from './WalkwayEditor';
...
function MapboxMapScreenInner({ visible, onClose, mapData, mapConfig, targetPoi }: MapboxMapScreenProps) {
  ...
  // Dev-mode simulated GPS location
  const {
    simulatedLocation,
  ...
  // Directions — client-side Dijkstra on park walkway graph
  const { route, routeInfo, fetchRoute, clearRoute, poiCoordinate } = useDirections(mapData, mapConfig, toCoord);

  // Dev-mode walkway editor
  const walkwayEditor = useWalkwayEditor(mapConfig);
  ...
  // Handle map background tap → editor intercepts when active, otherwise deselect
  const handleMapPress = useCallback((event: any) => {
    // Let walkway editor handle tap first
    if (walkwayEditor.handleEditorTap(event)) return;
  ...
  }, [selectedPoi, clearRoute, walkwayEditor.handleEditorTap]);

  // Handle map long-press → set simulated GPS location (dev mode only)
  const handleMapLongPress = useCallback(
    (feature: any) => {
      if (!SIMULATION_ENABLED) return;
      const coords = feature?.geometry?.coordinates;
      if (coords && coords.length >= 2) {
        setLocationFromMapEvent(coords[0], coords[1]);
      }
    },
    [setLocationFromMapEvent],
  );
  ...
  const handleNavigate = useCallback(
    (poi: ParkPOI) => {
      // Priority: simulated location > real GPS > entrance POI fallback
      let origin: [number, number];
      let originId: string | undefined;

      if (isSimulating && simulatedLocation) {
        // Dev mode: use the simulated tap location as origin
        origin = [simulatedLocation.longitude, simulatedLocation.latitude];
      } else if (isNearPark && userLocation) {
    ...
    [mapData.pois, fetchRoute, poiCoordinate, isNearPark, userLocation, isSimulating, simulatedLocation],
  );
    ...
    <View style={styles.overlay} pointerEvents={visible ? 'auto' : 'none'}>
      ...
      <Animated.View style={[styles.content, contentStyle]}>
        ...
        >
          ...
          {/* Dev mode: simulated GPS blue dot */}
          {SIMULATION_ENABLED && (
            <SimulatedLocationMarker location={simulatedLocation} />
          )}

          {/* Dev mode: walkway editor layers */}
          {SIMULATION_ENABLED && (
            <WalkwayEditorLayers
              nodes={walkwayEditor.nodes}
              edges={walkwayEditor.edges}
              polygonCorners={walkwayEditor.polygonCorners}
            />
          )}
        </MapboxGL.MapView>
        ...
        {/* Dev mode: location simulation banner */}
        {SIMULATION_ENABLED && (
          <DevLocationBanner
            simulatedLocation={simulatedLocation}
            onReset={clearSimulatedLocation}
            insetTop={insets.top}
          />
        )}
        ...
        {/* Dev mode: walkway editor toolbar */}
        {SIMULATION_ENABLED && (
          <WalkwayEditorToolbar
            editorActive={walkwayEditor.editorActive}
            toggleEditor={walkwayEditor.toggleEditor}
            nodeCount={walkwayEditor.nodes.length}
            edgeCount={walkwayEditor.edges.length}
            mode={walkwayEditor.mode}
            onSetModeZone={walkwayEditor.setModeZone}
            onSetModeCluster={walkwayEditor.setModeCluster}
            onSetModeBridge={walkwayEditor.setModeBridge}
            clusterSize={walkwayEditor.clusterSize}
            onCycleCluster={walkwayEditor.cycleClusterSize}
            currentZoneId={walkwayEditor.currentZoneId}
            onNewZone={walkwayEditor.newZone}
            polygonCornerCount={walkwayEditor.polygonCorners.length}
            onFillPolygon={walkwayEditor.fillPolygon}
            onUndo={walkwayEditor.undo}
            onClear={walkwayEditor.clearAll}
            onSave={walkwayEditor.save}
          />
        )}
      </Animated.View>
    </View>
