/**
 * LogConfirmSheet — Bottom-sheet log confirmation
 *
 * CoasterSheet-style bottom sheet for confirming ride logs.
 * Replaces the old centered-card overlay with a full portrait
 * hero image display and consistent sheet language.
 *
 * Animation sequence after "Log It":
 * 1. Button fades (250ms)
 * 2. Green checkmark pops up smoothly (no bouncing)
 * 3. "Logged!" text fades in
 * 4. Holds for ~1s
 * 5. Checkmark fades, rate nudge appears
 * 6. 5-second countdown → auto-dismiss
 */
...
// How far content lifts (translateY) — single lift for both celebration + rate nudge
...
export function LogConfirmSheet({
  ...
  const detailItems = useMemo(() => {
    ...
    if (coasterData.propulsion) items.push({ label: 'Propulsion', value: coasterData.propulsion });
    if (coasterData.status) items.push({ label: 'Status', value: coasterData.status });
    return items;
  }, [coasterId]);

  const richDetails = useMemo(() => COASTER_DETAILS[coasterId] ?? null, [coasterId]);

  const hasPage2 =
    secondaryStats.length > 0 ||
    detailItems.length > 0 ||
    !!richDetails?.description ||
    (richDetails?.notableFeatures && richDetails.notableFeatures.length > 0) ||
    (richDetails?.records && richDetails.records.length > 0);
  ...
  }, [onDismiss, sheetHeight, tabBar, clearTimers]);
  ...
  const handleRate = useCallback(() => {
    ...
    tabBar?.showTabBar();
    translateY.value = withTiming(sheetHeight, { duration: 300 }, (finished) => {
      if (finished) runOnJS(onRate)();
    });
    backdropOpacity.value = withTiming(0, { duration: 250 });
  }, [onRate, sheetHeight, tabBar, clearTimers]);

  // ── Pan gesture dismiss (confirm phase only) ──
  const panGesture = Gesture.Pan()
    .enabled(visible && phase === 'confirm')
    .onUpdate((e) => {
      'worklet';
      ...
      <Animated.View style={[StyleSheet.absoluteFill, backdropAnimStyle]}>
        <BlurView intensity={25} tint="light" style={StyleSheet.absoluteFill}>
          <Pressable
            ...
            onPress={phase === 'confirm' ? () => { haptics.tap(); dismiss(); } : undefined}
          />
        </BlurView>
      </Animated.View>
      ...
      >
        ...
        <View style={[styles.content, { paddingBottom: insets.bottom + 8 }]}>
          ...
          <Animated.View style={headerStyle}>
            <Text style={styles.name} numberOfLines={2}>{coasterName}</Text>
            ...
            {rideNumber > 1 && phase === 'confirm' && (
              <View style={styles.rideBadge}>
                ...
                <Text style={styles.rideBadgeText}>Ride #{rideNumber} today</Text>
              </View>
            )}
          </Animated.View>
                    ...
                    <View style={{ width: measuredPagerW, height: '100%' }}>
                      <View style={styles.page2Card}>
                        ...
                        >
                          ...
                          {richDetails?.notableFeatures && richDetails.notableFeatures.length > 0 && (
                            <View style={styles.p2Section}>
                              <Text style={styles.p2SectionLabel}>NOTABLE FEATURES</Text>
                              <View style={styles.p2FeaturesCard}>
                                {richDetails.notableFeatures.map((f, i) => (
                                  <View
                                    key={i}
                                    style={[
                                      styles.p2FeatureRow,
                                      i < richDetails.notableFeatures!.length - 1 && styles.p2FeatureRowBorder,
                                    ]}
                                  >
                                    <View style={styles.p2FeatureDot} />
                                    <Text style={styles.p2FeatureText}>{f}</Text>
                                  </View>
                                ))}
                              </View>
                            </View>
                          ...
                          {richDetails?.records && richDetails.records.length > 0 && (
                            <View style={styles.p2Section}>
                              <Text style={styles.p2SectionLabel}>RECORDS</Text>
                              {richDetails.records.map((r, i) => (
                                <View key={i} style={styles.p2RecordCard}>
                                  <Text style={styles.p2RecordText}>{r}</Text>
                                </View>
                              ))}
                            </View>
                          )}
          ...
          {/* Action Area — button lifts with content, celebration/nudge stay anchored */}
          <Animated.View style={[styles.actionArea, actionStyle]}>
            <Animated.View style={buttonAnimStyle} pointerEvents={phase === 'confirm' ? 'auto' : 'none'}>
              <Pressable
          ...
          {/* Celebration + Rate nudge — pinned to bottom of sheet, don't lift */}
          <View style={styles.bottomAnchor} pointerEvents="box-none">
            ...
            <Animated.View
              ...
              pointerEvents={phase === 'rate-nudge' ? 'auto' : 'none'}
            >
              ...
              <Pressable onPress={() => { haptics.tap(); dismiss(); }}>
                <Text style={styles.skipText}>Maybe later ({countdown})</Text>
              </Pressable>
            </Animated.View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  // -- Bottom anchor for celebration + nudge (stays in place while content lifts) --
  bottomAnchor: {
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 0,
    // Covers the lift space + faded button + action margin
  ...
  // -- Rate nudge -- centered in the full empty space, nudged up
  nudgeContainer: {
    position: 'absolute',
    left: 0,
    right: 0,
  ...
  p2StatCard: (() => {
    const size = Math.floor((SCREEN_WIDTH - spacing.xl * 2 - spacing.sm * 3) / 4);
    return {
      width: size,
      height: size,
      backgroundColor: colors.background.page,
